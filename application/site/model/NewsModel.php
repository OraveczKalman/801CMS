...
class NewsModel {
    ...
    private function getNews() {
        $getNewsQueryString = 'SELECT main_header.*, text.Text, cimlap_Kep.ProfilePicture FROM main_header ' .
            'LEFT JOIN rank ON main_header.MainHeaderId = rank.MainHeaderId ' .
            ...
            'WHERE rank.ParentId = ' . $this->headerData[0]['MainHeaderId'] . 
            ' AND text.Type = 1 AND main_header.`Active` = 1 ORDER BY Created DESC LIMIT ' . $this->headerData[0]['page'] . ', ' . $this->headerData[0]['limit'];
    ...
    public function getNewsCountByParent() {
        $getDocumentCountQueryString = 'SELECT COUNT(main_header.MainHeaderId) AS actNewsCount FROM main_header ' .
            'INNER JOIN rank ON main_header.MainHeaderId = rank.MainHeaderId ' .
            'WHERE rank.ParentId = "' . $this->headerData[0]['MainHeaderId'] . '" AND rank.Active = 1';
